...
Agents should use `agenttalk alert` when they need human attention but do not
need to assign work to another agent. The Hub records the alert and sends a
Feishu alert card when the Hub is started with Feishu enabled and
`FEISHU_ALERT_CHAT_ID` configured.

For multi-line alert content generated by the agent, use stdin:

```bash
agenttalk alert --from <agent-id> --type warning --message - <<'EOF'
Need human review: <short reason>
Evidence: <file, command, message id, or failure>
